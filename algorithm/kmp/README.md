...
서로 문자가 다르니까 `j = F[j - 1] = F[2 - 1] = F[1] = 1`
...
<!-- ```
i = 8 & j = 8 일때 문자열이 일치하지 않았어요. 그러면 그 전까지는 맞혔다는 것을 알고 있어요. 그 이유는 바로 j > 0 때문입니다.  
만약에 j = 0 이라면 그 전까지도 문자가 서로 다르다는 것을 이해하셔야 합니다. 본문으로 다시 돌아와 i, j가 0 ~ 7 일 때까지는 서로 같았어요.  
즉, 실패함수를 F(8 - 1) = F(7)를 구하면 됩니다. F(7) = 6은 무슨 의미일까요? 접두사 6의 길이와 접미사의 6의 길이가 같다는 것입니다.  
또한, 문자열의 i와 패턴의 j가 0 ~ 7까지 서로 같습니다. 그러면 문자열의 접미사의 6의 길이와 패턴의 접두사의 6의 길이가 서로 같다는 것을 알 수 있습니다.
그래서, j가 0 ~ 5까지 이미 같다는 정보를 확인해 j를 6으로 옮겨 다시 비교를 시작합니다.
``` -->
